...
This is a demonstration agent showing how to integrate OpenCode hooks with the Multi-Agent Observability system.

## Setup

1. Copy the `.opencode` directory to your OpenCode project
2. Update the `--source-app` parameter in `.opencode/settings.json` to identify your agent (currently set to `demo-agent`)
...
You can test the hook integration by running OpenCode in this directory:

```bash
cd apps/demo-opencode-agent
opencode -p --verbose --model opencode-pro --output-format stream-json "read the README.md and run ls" > "opencode-output.json"
```
...
- **Notification**: When OpenCode needs user input (with optional TTS support)
- **Stop**: When OpenCode completes a response (includes chat history)
...
### Custom Commands
The `.opencode/commands/` directory contains helpful slash commands:
