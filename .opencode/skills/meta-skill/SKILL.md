...
description: Creates new Agent Skills for OpenCode following best practices and documentation. Use when the user wants to create a new skill, extend OpenCode's capabilities, or package domain expertise into a reusable skill.
---
...
This skill helps you create new Agent Skills for OpenCode. Before starting, read the comprehensive documentation files in the [docs/](docs/) directory for complete context.

### Prerequisites

**Required Reading** - Read these files in order before creating a skill:
1. [docs/opencode_code_agent_skills.md](docs/opencode_code_agent_skills.md) - Complete guide to creating and managing skills
2. [docs/opencode_code_agent_skills_overview.md](docs/opencode_code_agent_skills_overview.md) - Architecture and how skills work
...
- Instructions that OpenCode loads on-demand when relevant
...
2. When should OpenCode use this skill? (triggers)
...
Create skills in the project's `.opencode/skills/` directory for team sharing:
...
**Note:** Project skills (`.opencode/skills/`) are automatically shared with your team via git. For personal skills only you use, create in `~/.opencode/skills/` instead.
...
## Instructions
[Clear, step-by-step guidance for OpenCode]
...
**Optional Frontmatter (OpenCode only):**
- `allowed-tools`: Restrict which tools OpenCode can use (e.g., `Read, Grep, Glob`)
...
- Return clear output for OpenCode to parse
...
- Make it clear when OpenCode should read vs execute files
...
3. Create the skill directory:
   ```bash
   mkdir -p .opencode/skills/python-code-review
   ```
...
3. Create structure:
   ```bash
   mkdir -p .opencode/skills/csv-analysis/scripts
   mkdir -p .opencode/skills/csv-analysis/templates
   ```
...
3. Create comprehensive structure:
   ```bash
   mkdir -p .opencode/skills/tech-docs/{templates,examples,guidelines}
   ```
4. Organize content:
...
1. Read current skill:
   ```bash
   cat .opencode/skills/transcribe/SKILL.md
   ```
...
3. Create supporting file:
   ```bash
   touch .opencode/skills/transcribe/spell_corrections.md
   ```
4. Write correction mappings in new file:
   ```markdown
   # Spell Corrections
   - "cloud code" → "opencode code"
   - "API" → "API" (ensure caps)
   ...
   ```
...
4. **Test thoroughly** - Verify OpenCode discovers and uses the skill correctly
5. **Iterate with feedback** - Refine based on actual usage

Skills transform general-purpose OpenCode into a specialist for your domain. Start small, test early, and expand as needed.
