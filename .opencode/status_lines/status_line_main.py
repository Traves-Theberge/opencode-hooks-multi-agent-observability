...
def get_session_data(session_id):
    ...
    session_file = Path(f".opencode/data/sessions/{session_id}.json")
...
def generate_status_line(input_data):
    ...
    model_name = model_info.get("display_name", "OpenCode")
...
def main():
    try:
        ...
        print("\033[31m[Agent] [OpenCode] 💭 JSON Error\033[0m")
        sys.exit(0)
    except Exception as e:
        # Handle any other errors gracefully - output basic status
        print(f"\033[31m[Agent] [OpenCode] 💭 Error: {str(e)}\033[0m")
        sys.exit(0)
